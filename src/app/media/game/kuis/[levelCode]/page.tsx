...
export default async function KuisGradePage(props: { params: Promise<{ levelCode: string }> }) {
  ...
  return (
    <Section className="min-h-[80vh] py-16 md:py-24">
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <Link href="/media/game/kuis" className="inline-flex items-center text-gray-500 hover:text-primary dark:text-gray-400 dark:hover:text-gold transition-colors">
        ...
        <div className="space-y-4 border-b border-gray-200 dark:border-gray-800 pb-8">
          <h1 className="font-title text-3xl font-bold text-secondary dark:text-white md:text-4xl tracking-wider">
            Kuis Interaktif {level.name} ({level.code})
          </h1>
          <p className="font-desc text-gray-600 dark:text-gray-400">
            Pilih kelas untuk memulai kuis yang tersedia.
          </p>
        </div>
        ...
        {grades.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {grades.map((grade) => (
              <Link 
                key={grade.id} 
                href={`/media/game/kuis/${level.code.toLowerCase()}/${grade.code.toLowerCase()}`}
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs font-semibold text-primary dark:text-gold mt-1 group-hover:underline">
                    Mulai Kuis &rarr;
                  </div>
                </div>
              </Link>
